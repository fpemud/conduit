...
class CUtil:
    ...
    def forceUnmount(mntDir):
        for i in range(0, 10):
            rc, out = CUtil.shell("/bin/umount %s" % (mntDir), "retcode+stdout")
            if rc == 0:
                return
            time.sleep(1.0)
        CUtil.shell("/bin/umount %s" % (mntDir))
    ...
    def copyToDir(srcFilename, dstdir, mode=None):
        ...
        if mode is not None:
            CUtil.shell("/bin/chmod " + mode + " \"" + fdst + "\"")
    ...
    def copyToFile(srcFilename, dstFilename, mode=None):
        ...
        if mode is not None:
            CUtil.shell("/bin/chmod " + mode + " \"" + dstFilename + "\"")

    @staticmethod
    def getDirFreeSpace(dirname):
        """Returns free space in MB"""

        ret = CUtil.shell("/bin/df -BM \"%s\"" % (dirname), "stdout").decode("ascii")
        m = re.search("^.* + [0-9]+M +[0-9]+M +([0-9]+)M +[0-9]+% .*$", ret, re.M)
        return int(m.group(1))

    @staticmethod
    def getLoopDevByFile(filename):
        ret = CUtil.shell("/sbin/losetup -j \"%s\"" % (filename), "stdout").decode("ascii")
